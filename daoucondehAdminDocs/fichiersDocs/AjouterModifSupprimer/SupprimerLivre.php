...
<body>
    ...
    unlink ("../../../livres/$nom_class_supp/$nom_matiere_supp/$image_fichier_supp");

    $filename = '../../../livres/$nom_class_supp/$nom_matiere_supp/$nom_fichier_supp';
    unlink ("../../../livres/$nom_class_supp/$nom_matiere_supp/$nom_fichier_supp");


    // la suppression dans la base de donne
    $req = $connexion->prepare("DELETE FROM livres WHERE id = ?");
	$req->execute(array($id));
    ?>

    <div class="btn_suppression">
        <h4>Supprission effectuée</h4>
        <button type="button" class="btn btn-success"><a href="../livre.php">Retour</a></button>
    </div>
